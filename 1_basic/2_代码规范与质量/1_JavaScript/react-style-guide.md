> 对于 react 的编码规范, 遵循 [Airbnb React/JSX Style Guide](https://github.com/airbnb/javascript/tree/master/react), 对应页面有相应的翻译版本。  
>
...
## 3. 数据模拟
 这一点应先和后端人员进行协商出格式。生成模拟数据的比较简单的一种方式是 mock.js

## 4. 样式
如何组织样式文件：
cssModule or styleComponent ? 这个看项目实际大小而定，进行模块化或者组件化，防止多个style在合成一个css文件后出现多份相互影响。

## 5. code split
 (按需加载的必要性？实现？) 看实际项目大小进行扩展。

## 6. 单元测试与集成测试

## 7. 数据
永远怀疑后端给的数据。就是说不要理所当然的想成返回的数据是完整的或者正确的，如某个字段缺失等。所以考虑错误情况。
